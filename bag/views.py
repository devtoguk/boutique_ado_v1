...
def add_to_bag(request, item_id):
    ...
    if size:
        ...
        if item_id in list(bag.keys()):
            ...
            bag[item_id] = quantity
            print(f'MESSAGE: Added {product.name} to your bag')
...
def adjust_bag(request, item_id):
    ...
    bag = request.session.get('bag', {})  # get bag from session or create empty {} bag
    ...
    if size:
        if quantity > 0:
            ...
            if not bag[item_id]['items_by_size']:  # remove whole item if sizes now empty
                bag.pop(item_id)
    else:
        if quantity > 0:
            bag[item_id] = quantity
